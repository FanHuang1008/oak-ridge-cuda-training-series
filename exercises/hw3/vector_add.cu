...
__global__ void vadd(const int ds, const float *A, const float *B, float *C) {
  // grid-stride loop
...
int main() {
  ...
  int num_blocks_per_grid = 160;
  int num_threads_per_block = 1024;
  vadd<<<num_blocks_per_grid, num_threads_per_block>>>(DSIZE, d_A, d_B, d_C);
